...
public class ClientSendMessageHandler
  ...
  public APIGatewayProxyResponseEvent handleRequest(APIGatewayProxyRequestEvent event, Context context) {
    try {
      ...
      // Read the request body; client does not provide sender type here
      Map<String, Object> requestBody = mapper.readValue(event.getBody(), Map.class);
      String content = (String) requestBody.get("content");

      // Build the message (sender is always the client)
      Message message = new Message();
      message.setMessageId(UUID.randomUUID().toString());
      message.setSender(clientId);
      message.setReceiver(therapistId);
      message.setContent(content);
      message.setCreatedAt(Instant.now().toString());

      // Prepare DynamoDB item (using conversationId as partition key and createdAt as
      // sort key)
      ...
      context.getLogger().log("Error in ClientSendMessageHandler: " + e.getMessage());
